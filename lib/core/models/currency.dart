/// A currency the app can convert between.
///
/// Equality is by [code] alone — two values with the same code are the same
/// currency regardless of how they were constructed.
class Currency {
  const Currency({
    required this.code,
    required this.name,
  ...
  /// How many decimals this currency is quoted to. Yen and Won use 0; the
  /// Gulf dinars use 3. Drives both display rounding and keypad input limits.
...
/// Every currency the app offers, in display order.
...
/// Offline fallback, USD-relative (1 USD = X). Used before the first
/// successful fetch and merged under live rates so a feed with narrower
/// coverage still renders every currency in the list.
const Map<String, double> kSeedRates = {
  'USD': 1.0,
  'EUR': 0.9234,
  'GBP': 0.7867,
  'JPY': 152.40,
